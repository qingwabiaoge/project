<template>

  <div
    :style="{background:`url(${$store.state.global.background3})`, backgroundSize:'cover',backgroundAttachment: 'fixed'} ">
    <div :class="$style['footer']">
      <div class="container">
        <el-row>
          <el-col :md="12">
            <h2>contact us</h2>

            <p>TEL:{{components.contact.tel}}</p>
            <p>
              {{components.brand.description|setLength(100)}}...
            </p>
            <h2>ADDRESS</h2>

            <p>
              {{components.contact.address}}
            </p>
          </el-col>
          <el-col :md="{span: 10, offset: 2}" :xs="{span: 24, offset: 0}">
            <h2>
              QUICK <br>
              CONTACT
            </h2>

            <el-form :model="message"
                     :rules="rules"
                     ref="message"

            >
              <!--prop="name"用于在validate时提示
             label视图中写在input之前的label-->
              <el-form-item>
                <!--message.name 数据-->
                <el-input v-model="message.name" placeholder=" Input a contact please"></el-input>
              </el-form-item>
              <el-form-item prop="tel">
                <!--message.name 数据-->
                <el-input v-model="message.tel" placeholder="Input a  phone number please"></el-input>
              </el-form-item>
              <el-form-item>
                <!--message.name 数据-->
                <el-input type="textarea" v-model="message.content" placeholder="Would you like to leave a message?"></el-input>
              </el-form-item>

              <h3 @click="submitForm('message')">SENT</h3>
            </el-form>

          </el-col>
        </el-row>
      </div>

      <footer>
        <div class="container">
          <el-row>

            <el-col :md="12">

              Guangzhou SENBOMA Artware Co.,Ltd Copyright@2019
            </el-col>

            <el-col :md="12">

              <section>
                <a :href="`mailto:${components.contact.email}`">
                  <svg t="1552138607639" class="icon" style="" viewBox="0 0 1024 1024" version="1.1"
                       xmlns="http://www.w3.org/2000/svg"
                       p-id="2138" xmlns:xlink="http://www.w3.org/1999/xlink">

                    <path
                      d="M960.000282 192.000282H64.000847c-35.346638 0-64.000094 28.653456-64.000094 63.999718v512.000376c0 35.346262 28.653456 63.999718 64.000094 63.999718h895.999435c35.347014 0 64.00047-28.653456 64.000471-63.999718V256c0-35.346262-28.653832-63.999718-64.000471-63.999718z m32.000612 63.999718v512a32.002117 32.002117 0 0 1-1.195117 8.658577 15.944049 15.944049 0 0 0-4.154311-5.364856l-314.253298-263.690367 314.253298-263.690367a16.05882 16.05882 0 0 0 2.049309-2.068124c2.109517 4.267952 3.300118 9.071751 3.300119 14.155137z m-32.000612-31.999859c0.195674 0 0.387961 0.011289 0.582507 0.014676l-406.769123 341.319912h-83.62545l-406.768747-341.319912c0.194169-0.003387 0.386456-0.014676 0.581378-0.014676h895.999435zM37.349663 771.293345a15.940286 15.940286 0 0 0-4.154311 5.364856 31.999483 31.999483 0 0 1-1.195117-8.658201v-512c0-5.083386 1.190601-9.886432 3.299742-14.154761a16.082903 16.082903 0 0 0 2.04931 2.068125l314.253297 263.68999-314.252921 263.689991z m26.650807 28.70689c-3.573686 0-7.006637-0.594548-10.215691-1.675271a16.002752 16.002752 0 0 0 4.133991-2.517798l318.575813-267.317111 75.299518 63.183908c2.934735 3.461173 7.312942 5.660625 12.206299 5.660625h95.999953c4.894109 0 9.27194-2.199452 12.206675-5.661378l75.298766-63.183155 318.575812 267.316735a15.994849 15.994849 0 0 0 4.133991 2.517798 31.935136 31.935136 0 0 1-10.216067 1.675647h-895.99906z"
                      fill="" p-id="2139"></path>
                  </svg>
                </a>

              </section>

            </el-col>


          </el-row>

        </div>
      </footer>
    </div>

  </div>
</template>

<script>

  export default {
    data() {
      return {
        message: {},
        rules: {
          tel: [
            {required: true, message: 'Enter your mobile phone number please', trigger: 'blur'},
          ],
        }
      }
    },
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$axios.post('/addMessage', this.message)
          } else {
            console.log('error submit!!');
            return false;
          }
        });
      },
    }
  }
</script>


<style lang="less" module>

  .footer {
    margin-top: 90px;
    padding: 90px 20px 0px;
    background: rgba(35, 35, 35, 0.91);

    h2 {
      font-size: 60px;
      text-transform: uppercase;
      color: #ccc;
    }
    * + h2 {
      margin-top: 80px
    }
    h3 {
      font-size: 60px;
      color: #444;
      &:hover {
        color: #ccc;
      }
    }
    p {
      color: #ccc;
      font-size: 16px;
      line-height: 2;
    }
    * + p {
      margin-top: 40px
    }
    form {
      margin-top: 50px;
    }
    input, textarea {
      border: 1px solid #777;
      height: 50px;
    }
    textarea {
      height: 150px
    }
    footer {
      border-top: 1px solid #777;
      margin-top: 180px;
      line-height: 108px;

    }
    section {
      text-align: right;
      svg {
        margin-left: 47px;
      }
      a {
        color: #555;
      }
    }
  }

</style>
