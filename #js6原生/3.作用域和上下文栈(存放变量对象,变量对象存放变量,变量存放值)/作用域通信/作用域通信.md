...
| \                            | 块作用域             | 函数作用域                              | global            | Script                 | 闭包                   | es6-module                                                   | node-module                         |
| ---------------------------- | -------------------- | --------------------------------------- | ----------------- | ---------------------- | ---------------------- | ------------------------------------------------------------ | ----------------------------------- |
| 隔离变量类型                 | const let            | const let var function                  | 作为全局作用域    | const let class import | const let var function | const let var  functionclass import                          | const let var  functionclass import |
|                              |                      |                                         |                   |                        |                        |                                                              |                                     |
| 自由变量(作用域链)           | √                    | √                                       | √                 | √                      | √                      | √                                                            | √                                   |
| 参数                         | x                    | fn( p1,callback),作为数据的入口和出口   | x                 | x                      | √                      | x                                                            | x                                   |
| 传入this变量(赋值对象的地址) | 继承函数作用域的this | 箭头函数继承this,<br />普通函数指定this | window和undefined | undefined              | 指定this               | undefined                                                    | global                              |
|                              |                      |                                         |                   |                        |                        |                                                              |                                     |
| 输出关键字                   | do(浏览器未支持)     | return i (同步用于计算,异步函数通信)    | x                 | x                      | return{ }              | export{ }<br />export default                                | modules.export                      |
...
###### 先声明变量再引入变量到引入方文件作用域+注入到对象(方便通信)
