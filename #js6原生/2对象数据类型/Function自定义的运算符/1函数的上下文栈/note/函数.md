...
- 变量里存基本类型和引用类型地址
 ![]( img/2.png)
 <center> 堆,栈,VO(变量对象),AO(活动对象),变量,基本类型值,引用类型值关系图</center>
### 堆

#####  结构

* 堆可以任意放置的
* 堆存放每个对象,可以看成树结构(通过键值对一分多)
##### 存放
...
>
> __fn保存着内存地址,fn和()计算运行__只有函数调用,才会有变量对象进入函数上下文栈,才会初始化函数体内的局部变量,然后cpu执行函数作用域里的代码
...
var i = 1
fn2(function fn() {//1函数声明 自由变量在哪个作用域取值在声明时间已经确定
    console.log(i)
})
...
js代码使用静态作用域,  是因为fn.__ parent__===函数定义式所在的父作用域 
...
可使用__ parent__(为函数定义式所在的父作用域)的变量,
