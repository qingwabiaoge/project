...
(存放变量对象,变量对象存放变量,变量里存基本类型和引用类型地址)
...
> 打开页面代码顺序(包含import里的代码)调用执行(执行运算符|函数运行表达式),只有函数调用,才会建立函数上下文栈
...
fn:0x000001, // 一.开辟堆内存空间0x000001 赋值给fn,二.如果此时有同名的变量fn 函数fn先初始化,程序会优先访问0x000001
...
6. 箭头函数内部this继承其父函数的this,类似作用域链

作用:为了方便使用上级函数的this才引入的



7. method（this）不算method函数作用域 是操作符
