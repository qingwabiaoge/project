...
|                                     |                  |                      |                                                              |                        |                         |                                                              |                                     |
| 输出关键字                          | x                | do(浏览器未支持)     | return i (==异步函数return作为出口==) 例如const i=fs.writeFileasync(path) | x                      | ==return{ }==           | ==export{ i,fn}<br />export default{i,fn}==                  | ==modules.export==                  |
...
## async function\es6-module\node-module\closure出入口通信
