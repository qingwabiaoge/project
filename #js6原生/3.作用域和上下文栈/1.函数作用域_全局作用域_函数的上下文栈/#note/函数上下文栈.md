...
const f=obj.fn;    f()    先得到函数地址赋值给变量 ,赋值给f再执行 
...
```javascript
//下边3个不会注入到global,存放在在一个叫Script/module的作用域里,js变量开始和顶级环境分离
      let i = 1,
      const m=2
      class person{}
      import n from './'
 //store全局变量,为方便this.store调用注入到对象中
      ...
      function g_fn(p1, p2) {//g_obj.g_fn(1, 2)后fn-vo进栈
          ...
          //1,跨作用域链寻找自由,模仿栈内寻找变量对象-----------------------------
          console.log(i)
          // 2.自由变量声明,也可以提供给下级的作用域使用---------------------------
        
          function fn() {   console.log(this)}// 1.提前进栈准备声明变量 2.覆盖同名变量3.赋值
          let a = 1  //let const暂时性死区
      ...
      console.log(m)// 10,因为注入到了global,global是全局的永远不出栈
      n()// 对内存中的地址未销毁,window.m===#0x00001
...
a:占位符 //let变量 a为占位符没有赋值undefined,还会造成暂时性死区 
...
4. prototype的this也是实例化对象本身,原理是`obj.__proto__.xxx`
