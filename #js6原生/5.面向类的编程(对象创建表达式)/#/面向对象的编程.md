...
②重载是指不同的函数使用相同的函数名，但是函数的 **参数个数或类型** 不同。调用的时候根据函数的参数来区别不同的函数。
...
### js原生集成- 原型链模式，通过函数实现继承

##### 原生继承的实现

js继承是子类通过copy父类的属性和方法完成的,如何完成的呢?

如果Sub.prototype深copy一份父Sup.Prototype，复用代码变得无意义

如果子类浅copy父类的属性和方法,这样连Sup.Prototype也改变了，

于是乎用Js创造的特殊属性`__proto__`来完成，`Sub.Prototype.__proto__=Sup.Prototype`,因为`__proto__`是隐藏属性不能直接设置属性，可以 实现通过`Sub.Prototype=new Sup（）`或`Sub.Prototype=Object.Creat(Sup.Prototype)`实现,后者优于前者，

而构造函数那借鸡生蛋的call和apply和继承无关
...
function inheritPrototype (subType, superType) {
  //*1.效果等价于等价于_prototype={__proto__:superType.prototype}*/
...
//不可更改Man.prototype引用的内存地址,例如Man.prototype={.....}
...
对象类型<=>Object子类的实例<=>引用类型(内存地址)<=>`{ownprop:'',ownmethod(){},__proto__:{....}}形式`
...
***各种关系的强弱顺序：***泛化 = 实现 > 组合 > 聚合 > 关联 > 依

**一般来说类与类有六种关联方式**：单向关联，双向关联，自关联，多重性关联，聚合关系，组合关系。

| 关系                                                         | 代码                                                         |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| class A-->classB                                             | classA里的局部变量 方法的参数的类型有class B类型的           |
| class A->classB                                              | classA的属性类型为class B类型的                              |
| class A◇->classB<br /> （#空三角表示继承和显示  代码里 #便是只给之类继承的字段） | classA的属性类型为class B类型的，classB是聚合class A的一部分 |
| class A◆->classB                                             | classA的属性类型为class B类型的，classB是组合成class A一部分，classB脱离了classA就毫无意义了 |
| class A--▷interfaceB                                         | classA的原型是interfaceb类型的                               |
| class A-▷classB                                              | classA的原型是classb类型的                                   |
|                                                              |                                                              |

##### 依赖关系

【关系】：是一种使用的关系，即一个类的实现需要另一个类的协助，所以要尽量不使用双向的互相依赖.可以简单的理解，就是一个类A使用到了另一个类B，而这种使用关系是具有偶然性的、、临时性的、非常弱的，     但是B类的变化会影响到A；比如某人要过河，需要借用一条船，此时人与船之间的关系就是依赖；
【代码表现】：为类B作为参数被类A 局部变量、方法的参数或者对静态方法的调用
![img](img/面向对象的编程/Dependence.jpg)

##### 关联关系

![img](img/1539813-20190721132552390-1322769878.png)

【关系】：是一种拥有的关系，它使一个类知道另一个类的属性和方法；如：老师与学生，丈夫与妻子关联可以是双向的，也可以是单向的。

【代码表现】：成员变量

1单向关联一般是把B类作为A类的一个==属性==来实用，例如Company是一个类，Employee是一个类。然而Company类将Employee作为其的一个属性。

![img](img/20190219223619608.png)

2双向关联，就是说将对方（类）当做自己的一个属性。例如顾客与商品的关系，顾客购买商品，卖出的商品与某个顾客关联。

![img](img/20190219231836107.png)

3.自关联的比如链表中的节点，可以指向自己，把自己当做自己类的一个属性。

![img](img/20190219230857478.png)

4.多重性关联，即A类把多个B类当做属性。

![img](img/20190224001034296.png)
...
​    【Visio2010】：复合，聚合类型为共享，没有箭头
 ...
 ![img](img/20190224001034296-1585990472128.png)![img](https://img2018.cnblogs.com/blog/1539813/201907/1539813-20190721131412086-48381944.png)
...
​    【Visio2010】：复合，聚合类型为复合，没有箭头
 ...
 ![img](img/20190224001724815.png)![img](img/1539813-20190721131417944-1261569305.png)
...
![img](img/1539813-20190721131823088-1716546917.png)
...
![img](img/1539813-20190721111222599-1690181836.png)
...
总体图例子：
 ...
 ![img](img/2799767-3f16972d7b062110-1585991046473.png) 
...
![image-20200407184435184](img/面向对象的编程/image-20200407184435184.png)
...
![img](img/面向对象的编程/44190429-639a0680-a159-11e8-88f8-765517d2b5fa-1586256653402.png)

##### Promise

![img](img/44190477-947a3b80-a159-11e8-8e3a-0afde3f3d01b.png)



![img](img/44190438-6c8ad800-a159-11e8-9a48-c319526b6ad2.png)



##### jQuery callbacks

![img](img/44190487-a22fc100-a159-11e8-8ccc-1bdb595a4bab.png)
...
![img](img/44190524-cb505180-a159-11e8-85bc-8c2ece27789e.png)

##### vue watch

![img](img/44190533-d4d9b980-a159-11e8-8829-d3054ba1ba9c-1586258355147.png)
