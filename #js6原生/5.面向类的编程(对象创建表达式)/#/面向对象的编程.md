# ![img](img/2799767-3f16972d7b062110.png)做什么(函数)和谁去做(实例化对象)
...
js继承是通过copy完成的,如何完成的呢?

当然Sub.prototype总不能直接深copy一份父Sup.Prototype
浅copy稍后添加属性,这样连Sup.Prototype也改变了，

于是乎用Js创造的特殊属性`__proto__`来完成，`Sub.Prototype.__proto_=Sup.Prototype`

可以 实现通过new Sub（）或Object.Creat()实现,后者优于前者
...
0. 依赖(Dependency)：对象之间最弱的一种关联方式，是临时性的关联。代码中一般指由局部变量、函数参数、返回值建立的对于其他对象的调用关系。一个类调用被依赖类中的某些方法而得以完成这个类的一些职责。在类图使用带箭头的虚线表示，箭头从使用类指向被依赖的类。

![img](img/1071603-20171122100240086-1421148234.jpg)

1.单向关联一般是把B类作为A类的一个==属性==来实用，例如Company是一个类，Employee是一个类。然而Company类将Employee作为其的一个属性。
