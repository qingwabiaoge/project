...
| 描述       | 普通函数 | 构造函数                                           |
| ---------- | -------- | -------------------------------------------------- |
| 相同的部分 | 函数语句 | 属性语句,例如this.mame=name<br />prototype原型方法 |
| 不同的部分 | 函数参数 | 对象的属性值                                       |
...
| 描述                 |                                                  |      |
| -------------------- | ------------------------------------------------ | ---- |
| 重复使用的方法和属性 | __解耦成模块:__比如axios对象                     |      |
| 使用一次的属性和方法 | __内聚:__比如 vue实例的elementui表单验证规则对象 |      |
...
| .                                  | 块作用域             | 函数作用域                                | global            |
| ---------------------------------- | -------------------- | ----------------------------------------- | ----------------- |
| 隔离变量类型                       | const let            | const let var                             | 全局变量          |
|                                    |                      |                                           |                   |
| 作用域内声明自由变量               | √                    | √                                         | √                 |
| 参数                               | x                    | fn( p1,callback),作为数据的入口和出口     | x                 |
| 传入this变量(引用其他的对象的类型) | 继承函数作用域的this | 箭头函数继承this,<br />普通函数如传入this | window和undefined |
| 作用域链                           | √                    | √                                         | √                 |
|                                    |                      |                                           |                   |
...
| 引入暴露值到其他作用域                        | const {i,refn}=fn()      | import {i,fn}from './export' |
...
###### 3.进化版+注入,把const import声明的全局变量,再批量注入到对象的key里,通过`this`调用
