...
| 描述       | 普通函数 | 构造函数                       |
| ---------- | -------- | ------------------------------ |
| 相同的部分 | 函数语句 | this属性语句,prototype原型方法 |
| 不同的部分 | 函数参数 |                                |
...
| 作用域链继承上面三个               | √                    | √                                         | √                                         |
|                                    |                      |                                           |                                           |
| 暴漏(数据出口)                     | do(浏览器未支持)     | return                                    | exprort{ }                                |
| 全局变量的方式引入                 |                      | const i=fn()                              | import obj from './export'                |
| 跟随变化                           |                      | 跟随return变化同步变化                    | 跟随exprot的变量的变化而同步变化          |
| 暴漏赋值的同时执行代码             | 直接执行代码         | √                                         | √                                         |
...
###### 3.进化+注入,把const import声明的全局变量批量注入到每个对象的key里,通过`this`调用
...
| __本质__     | this.key做全局变量 | 函数参数 | global.route变量做全局变量                                   | $store变量做全局变量                                         |
| ------------ | ------------------ | -------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
