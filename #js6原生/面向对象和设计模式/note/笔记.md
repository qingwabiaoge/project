...
通过注入Construct.prototype到实例的__ proto __ 和运行 Construct作用与这个实例
...
```js
  ...
  var obj = new Object();
  obj.__proto__ = Person.prototype;
  var res = Person.call(obj, 'xiaohong', 20)// 改变obj的值
  if (typeof res === Object) {//判断function的返回值 若返回值为对象
    xiaohong = res
  } else { //如果返回值为undefine或者基础变量 构造函数成功
    xiaohong = obj
  }
...
##### 字面量等新建对象和控制台打印(等同于new Construct())
...
##### 返回的真实对象都是键值对都是`{prop:'',fn(){},__proto__}`的形式
...
##### 添加修改删除访问own属性(js弱类型键值对对象都可以再扩展和删除)
