...
//类(名词Dog).
//方法(函数,动词update),
//方法的参数(条件conditions,覆盖的数据update,回调函数)
//ts还有类型检查也算条件
Dog.update(conditions, update, callback)
...
Array.protorype.sort.call(arr,(a,b)=>a-b)//call(arr ),this===arr;回调参数做参数
```
### 封装（encapsulation ）
...
###### 弱类型js的实例化对象,若无构造函数参与,弱类型不受类控制,随意生成
