 ...
 **本质:**一种解耦机制,把做什么(函数体)和谁去做(强类型语言检查参数类型)解耦:
...
//不是所有浏览器都支持__proto__,不能直接设置subtype.__proto--=Supertype.protype,所以使用object()
    ...
    prototype.constructor = subType;// 补充原型加入构造函数{__proto__=superType.protype,constructor:SuperType}
