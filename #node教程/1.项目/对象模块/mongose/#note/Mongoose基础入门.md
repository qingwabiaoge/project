...
```js
//对象写法
userModel.find({'name':'张三'},{'name':1,'sex':1,'region':1,'createBy':1,'_id':0},{ limit:2, skip:1, sort:'-createBy.createTime'},callback)
//链式写法
...
```
temp.find({age: {$in: [10, 20]}}, callback)
```
...
```
Dog.findOne({ name: { $regex: 'huo' || '', $options: '$i' } }, callback)
```
