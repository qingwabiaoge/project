...
fs.open('../assets/1.txt', 'r', function (err, fd) {
  if (err) {
     ...
     * callback(err, readByte, buffer), 读取执行完成后回调函数，readByte实际读取字节数，被读取的缓存区对象
     */
    var buffer = new Buffer.alloc(255)
               //读入到buffer
    fs.read(fd, buffer, 0, 9, 3, function (err, readByte, buffer) {
      if (err) {
        console.log(err)
      } else {
        console.log(readByte)
        console.log( buffer.slice(0, readByte).toString())
      }
    })
  }
})
